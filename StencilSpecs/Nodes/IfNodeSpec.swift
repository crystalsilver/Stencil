...
describe("IfNode") {
  $0.describe("parsing") {
    $0.it("can parse an if block") {
      ...
      try expect(node?.leftArgument.variable) == "value"
    ...
    $0.it("can parse an if equal block") {
      let tokens = [
      ...
      try expect(nodes.count) == 1
      try expect(node?.leftArgument.variable) == "value"
      try expect(node?.rightArgument?.variable) == "otherValue"
      try expect(node?.trueNodes.count) == 1
      try expect(trueNode?.text) == "true"
      try expect(node?.falseNodes.count) == 1
      try expect(falseNode?.text) == "false"
  ...
  $0.describe("rendering") {
    ...
    $0.it("renders the truth when expression evaluates to true") {
      let node = IfNode(leftArgument: "items", trueNodes: [TextNode(text: "true")], falseNodes: [TextNode(text: "false")])
      try expect(try node.render(context)) == "true"
    }

    $0.it("renders the false when expression evaluates to false") {
      let node = IfNode(leftArgument: "not_so", trueNodes: [TextNode(text: "true")], falseNodes: [TextNode(text: "false")])
      try expect(try node.render(context)) == "false"
    }

    $0.it("renders the false when expression key is unknown") {
      let node = IfNode(leftArgument: "unknown_key", trueNodes: [TextNode(text: "true")], falseNodes: [TextNode(text: "false")])
      try expect(try node.render(context)) == "false"
    }

    $0.it("renders the false when expression evaluates to empty string") {
      let node = IfNode(leftArgument: "empty", trueNodes: [TextNode(text: "true")], falseNodes: [TextNode(text: "false")])
      try expect(try node.render(context)) == "false"
    }
    ...
    $0.it("renders the truth when equality evaluates to true") {
      let node = IfNode(leftArgument: "1", rightArgument: "1", comparisonOperator: .Equality, trueNodes: [TextNode(text: "true")], falseNodes: [TextNode(text: "false")])
      try expect(try node.render(context)) == "true"
    }

    $0.it("renders the false when equality evaluates to false") {
      let node = IfNode(leftArgument: "1", rightArgument: "2", comparisonOperator: .Equality, trueNodes: [TextNode(text: "true")], falseNodes: [TextNode(text: "false")])
      try expect(try node.render(context)) == "false"
    }
  }
}
